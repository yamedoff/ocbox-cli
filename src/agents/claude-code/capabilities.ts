...
export const COVERED_CAPABILITIES: readonly CapabilityRow[] = [
  {
    ...
    status: 'covered',
    detail: `Proven for the pinned version only (single pinned surface: ${CLAUDE_HOOK_EVENTS.length} hook events, ${CLAUDE_TOOL_MATCHERS.length} tool matchers, evidence ${CLAUDE_CODE_PINNED_SURFACE_EVIDENCE.fixture}). The owned hook entrypoint reads the PreToolUse stdin JSON (tool_input.command) and routes matching shell execution as a routing aid to the selected Session through "ocbox exec". After a routed run the hook emits Claude Code's documented PreToolUse blocking decision ({"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny",...}}) and exits 2, so the original local Bash call is explicitly denied and never executes a second time whether the remote command succeeded, exited nonzero (including 2), timed out, was cancelled, or failed after start. The denial reason is machine-readable: "ocbox-block[route] ... exit-code=<n>" reports the honest remote outcome, while an unroutable or unreadable covered call fails closed with "ocbox-block[guard]", so a legitimate remote exit 2 is never confused with the adapter's own block. A routed call exports OCBOX_AGENT_ROUTED=1 and OCBOX_AGENT_ADAPTER=claude-code so a nested adapter-owned call is left local instead of recursing. Live confirmation of this contract against the installed pinned binary (Claude Code ${PINNED_CLAUDE_CODE_VERSION}) remains a named AC9 live gate; this row is an offline implementation of the documented contract.`,
  },
  {
