...
import { CODEX_HOOK_TIMEOUT_SECONDS } from '../../../src/agents/codex/timeouts.js'
...
describe('strict ownership protects user lookalikes (D2)', () => {
  ...
  it('preserves unrelated user hooks byte-for-byte when a manifest is present', () => {
    ...
    // The drifted owned group must use the actually-installed shape (with the
    // pinned hook `timeout`): the manifest records the installed fragment
    // exactly, so a stale timeout-less group reads as a user-modified owned
    // fragment and correctly repairs instead of removing.
    const installedOwnedGroup = {
      matcher: CODEX_HOOK_MATCHER_TOOL,
      hooks: [
        {
          type: 'command',
          command: buildHookCommand({ sessionId: 'sess-old' }),
          timeout: CODEX_HOOK_TIMEOUT_SECONDS,
        },
      ],
    }
    const drifted = hooksJson({
      PreToolUse: [installedOwnedGroup, userExecLookalike, userRunLookalike],
      SessionStart: [userGroup],
    })
    const remove = planCodexRemove({
